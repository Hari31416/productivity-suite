...
describe('Brand Identity & Color System Verification', () => {
  ...
  it('configures light mode core brand and neutral tokens in index.css', () => {
    ...
    // Light secondary text #334155 (215 25% 27%)
    expect(indexCss).toContain('--secondary-foreground: 215 25% 27%')
    // Light muted text #475569 (215.3 19.3% 34.5%)
    expect(indexCss).toContain('--muted-foreground: 215.3 19.3% 34.5%')
  })
  ...
  it('configures dark mode tokens in index.css', () => {
    ...
    // Dark secondary text #BED0CC (165.9 16% 78%)
    expect(indexCss).toContain('165.9 16% 78%')
    // Dark muted text #96ABA6 (168 12% 64%)
    expect(indexCss).toContain('168 12% 64%')
